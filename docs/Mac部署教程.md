...
去不去翻墙由你自己的电脑决定，程序自己不带代理，所有请求都直连出去。

- **天气、音乐、Photon（iMessage 桥接）、国内的中转站**：直连就通，什么都不用做
- **Instagram 和 DuckDuckGo 搜索**：国内直连不通。要用的话开全局代理 —— Clash 这类客户端请开 **TUN 模式**（虚拟网卡）。只开「系统代理」那个开关不够，后台用的 Node 不认系统代理
