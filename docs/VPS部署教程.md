...
- **境外机器**：什么都不用做，Instagram 和联网搜索直连就通
- **境内机器**：天气、音乐、Photon（iMessage 桥接）、国内的中转站直连都通；**Instagram 和 DuckDuckGo 搜索不通**。要用 Instagram 得在服务器上跑一个 TUN 模式的全局代理（Clash Meta / mihomo 的 `tun.enable: true`），只设 `HTTPS_PROXY` 环境变量没用 —— Node 的 fetch 不认它。联网搜索可以直接换 Tavily / Brave
