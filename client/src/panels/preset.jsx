...
const GATED_CHILD_HINTS = {
  ...
  transfer: {
    switch: "转账卡片",
    why: "发出去的是一张长得像凭证的气泡 —— 角色跟你之间有没有钱来钱往，该你自己定",
  },
   ...
   *
   * 以前合成一条时这儿写的是「五个开关各管一摊」—— 拆开之后那句话就不对了：
   * 用户在「放歌」那条底下看到的提示，该说去开「让角色放歌」，提另外四个开关
   * 只会让他以为四个都得开。
   *
   * 屏幕那条仍然提两个，因为它正文里就是两行；只开一条时另一条的标签会被裁掉
   * （server/src/spy.js:trimSpyPrompt）。
   */
  spyScreen: {
    switch: "电脑查岗 / 手机查岗",
    why: "会把你屏幕上的东西抓下来打给视觉模型。两个都关的话这一条压根不注入，只开一条时另一条的标签会被裁掉",
  },
  spyView: {
    switch: "查看手机里的东西",
    why: "会替你打开微信、支付宝这些 App 再截屏 —— 具体哪几项能看，在那个开关底下还能一项项挑",
  },
  spyControl: {
    switch: "操控手机",
    why: "会真的动你的手机：定闹钟、关闹钟、锁屏 —— 具体哪几项能做，在那个开关底下还能一项项挑",
  },
  spyMusic: {
    switch: "让角色放歌",
    why: "会在你手机上打开网易云放歌 —— 每日推荐、私人漫游、点歌这几项在那个开关底下还能一项项挑",
  },
};
