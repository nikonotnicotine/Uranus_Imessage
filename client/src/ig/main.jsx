 ...
 * 这一页也在登录门后面。
 *
 * cookie 是按**主机**存的、不含端口，所以在控制台（8787）登录过之后这一页直接就是
 * 登录态，不用再登一次；反过来先打开这一页也一样。
 *
 * 为什么这一页也要门：它渲染的是角色和用户的帖子、快拍、评论 —— 那和聊天记录是
 * 同一类东西。而且 `/api/ig/*` 本来就在后端那道闸后面，不套门的话这一页只会卡在
 * 「拉不到数据：没有登录凭据」，用户还得自己猜该去哪。
 *
