...
const DEFAULT_FORMAT_CHILDREN = {
  ...
  /*
   * 查岗。五行标签都教给模型，**按角色开着哪几组裁剪是服务端的事**
   * （server/src/spy.js:trimSpyPrompt）—— 关掉那几组的行会被删掉，所以这份
   * 死文本里五行都写着，别在这儿按开关分叉。
   *
   * **必须和 server/src/preset.js:SPY_CHILD 一字不差。** 那边拿它当「用户没改过」
   * 的判据（normalizeFormatChildren 里的 legacyContent），差一个字就会把用户
   * 从没动过的正文当成他自己改的，以后再升级就不敢替他换了。
   *
   * 手机那三行里的 `{{查看项}}` / `{{操控项}}` / `{{网易云项}}` 是占位符，
   * 由服务端换成那一组真能用的功能清单 —— 别在这儿把功能名写死。
   */
  spy: [
    "      查岗:",
