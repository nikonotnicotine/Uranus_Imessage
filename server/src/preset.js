 ...
 * 查岗那一条的默认正文。
 *
 * **五行标签对着角色上五个开关**（server/src/spy.js:spyLegs）。这份死文本里
 * 五行都写着，按角色开着哪几组裁剪是服务端的事（spy.js:trimSpyPrompt）——
 * 别在这儿按开关分叉，这只是一段默认文字，用户还能自己改。
 *
 * 屏幕那两行靠标签字面量被认出来，手机里那三行靠 `{{查看项}}` /`{{操控项}}` /
 * `{{网易云项}}` 三个占位符（理由见 spy.js:GROUP_VARS —— 后两组共用
 * `[操控手机:…]` 这一个前缀，按标签认的话删一组会连着删掉另一组）。
 * 占位符会被换成那一组真正能用的功能清单，所以**这儿不许把功能名写死** ——
 * 写死了 spyfeatures.js 加一条，这段文字就过期了。
 *
 * 那三行的标签写成 `[查岗手机:项目名]` 这种带示例的形式，是因为这几个标签
 * 带参数，光给个空壳模型不知道冒号后面该填什么。
 */
const SPY_CHILD = [
  "      查岗:",
...
function normalizeFormatChildren(list) {
  ...
  const legacyContent = {
    ...
    // 只有屏幕两行的旧版查岗正文 → 换成带手机那三行的（见 LEGACY_SPY_CHILD）
    spy: LEGACY_SPY_CHILD,
  };
  return FORMAT_CHILD_KINDS.map((kind) => {
